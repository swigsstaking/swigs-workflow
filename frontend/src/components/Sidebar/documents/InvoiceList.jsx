...
import {
  Receipt, Plus, Send, Check, MoreVertical, Trash2, Mail, Bell, BellOff, ExternalLink, Link2, Clock, CheckCircle, XCircle, FileDown, Pencil, CreditCard, RotateCcw, Landmark
} from 'lucide-react';
...
export default function InvoiceList({
  ...
  useEffect(() => {
    if (!activeMenu?.startsWith('invoice-') && !activeMenu?.startsWith('reminder-')) setMenuPos(null);
  }, [activeMenu]);
  ...
  const handleReminderToggle = (e, invoiceId) => {
    const id = `reminder-${invoiceId}`;
    if (activeMenu === id) {
      setActiveMenu(null);
      setMenuPos(null);
    } else {
  ...
  return (
    <section>
      ...
      {(activeMenu?.startsWith('invoice-') || activeMenu?.startsWith('reminder-')) && createPortal(
      ...
      {invoices.length === 0 ? (
        <div className="flex flex-col items-center py-8 text-center border border-dashed border-[rgb(var(--swigs-stone)/0.4)] dark:border-dark-border rounded-[8px] bg-[rgb(var(--swigs-cream)/0.3)] dark:bg-white/[0.02]">
          ...
          <p className="text-xs text-slate-400 dark:text-zinc-500 mb-2">
            Ajoutez des heures ou dépenses, puis créez une facture
          </p>
          {!settings?.company?.iban && (
            <Link
              to="/settings?section=company"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 rounded-md hover:bg-primary-100 dark:hover:bg-primary-900/30 transition-colors"
            >
              <Landmark className="w-3 h-3" />
              Configurer l'IBAN pour les QR-factures
            </Link>
          )}
        </div>
      ) : (
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {activeMenu === `invoice-${invoice._id}` && menuPos && createPortal(
                      <div
                      ...
                      >
                        ...
                        {invoice.status === 'draft' && (
                          <button
                            onClick={() => { onEdit(invoice); closeMenu(); }}
                            className="w-full px-4 py-2 text-sm text-left text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-dark-hover flex items-center gap-2"
                          >
                        ...
                        {invoice.status === 'sent' && (
                          <button
                            onClick={() => { onStatusChange(invoice._id, 'draft'); closeMenu(); }}
                            className="w-full px-4 py-2 text-sm text-left text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-dark-hover flex items-center gap-2"
                          >
                        ...
                        {['sent', 'partial'].includes(invoice.status) && (
                          <button
                            onClick={() => { onRecordPayment(invoice); closeMenu(); }}
                  ...
                  return (
                    <div className="flex items-center justify-between gap-2 pl-8">
                      ...
                      <div className="flex items-center flex-wrap gap-1.5 min-w-0">
                        ...
                        {hasReminders && (<>
                          <button
                            onClick={(e) => handleReminderToggle(e, invoice._id)}
                          ...
                          {activeMenu === `reminder-${invoice._id}` && invoice.reminders?.length > 0 && menuPos && createPortal(
                            <div
                              className="fixed w-64 bg-white dark:bg-dark-card rounded-lg shadow-lg border border-slate-200 dark:border-dark-border p-3 z-[9999]"
                              style={{
                                left: menuPos.left,
                                ...(menuPos.bottom != null ? { bottom: menuPos.bottom } : { top: menuPos.top }),
                              }}
